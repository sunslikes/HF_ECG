...
#双层残差模块
def res_layer1d(input_tensor,kshape = [5,5],deph = 64,conv_stride = 1,padding='SAME', trainable = True):
    data = input_tensor
    data = slim.batch_norm(data, activation_fn=tf.nn.relu )

    #模块内部第一层卷积
    data = slim.conv1d(data,num_outputs=deph,kernel_size=kshape,stride=conv_stride,padding=padding, trainable=trainable)

    # #模块内部第二层卷积
    data = slim.conv1d(data,num_outputs=deph,kernel_size=kshape,stride=conv_stride,padding=padding,activation_fn=None, trainable=trainable)
...
#组合同类残差模块
def res_block(input_tensor,kshape,deph,layer = 0,half = False,name = None, trainable=True):
    data = input_tensor
    with tf.variable_scope(name):
        if half:
            data = get_half(data,deph//2, trainable=trainable)
        for i in range(layer//2):
            data = res_layer1d(input_tensor = data,deph = deph,kshape = kshape, trainable=trainable)
        return data
...
def inference(input_tensor, regularizer = None, trainable=True):
    with slim.arg_scope([slim.conv1d,slim.max_pool2d],stride = 1,padding = 'SAME'): # 没有max_pool1d
        ...
        with tf.variable_scope("layer1-initconv"):
            ...
            with tf.variable_scope("resnet_layer"):
                ...
                data = res_block(input_tensor = data,kshape = [CONV_SIZE, CONV_SIZE],deph = CONV_DEEP,layer = 6,half = False,name = "layer4-9-conv", trainable=trainable)
                data = res_block(input_tensor = data,kshape = [CONV_SIZE, CONV_SIZE],deph = CONV_DEEP * 2,layer = 8,half = True,name = "layer10-15-conv", trainable=trainable)
                data = res_block(input_tensor = data,kshape = [CONV_SIZE, CONV_SIZE],deph = CONV_DEEP * 4,layer = 12,half = True,name = "layer16-27-conv", trainable=trainable)
                data = res_block(input_tensor = data,kshape = [CONV_SIZE, CONV_SIZE],deph = CONV_DEEP * 8,layer = 6,half = True,name = "layer28-33-conv", trainable=trainable)
