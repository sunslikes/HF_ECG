...
#双层残差模块
def res_layer2d(input_tensor,kshape = [5,5],deph = 64,conv_stride = 1,padding='SAME', trainable = True):
    data = input_tensor
...
def get_half(input_tensor,deph, trainable = True):
    data = input_tensor
    data = slim.conv2d(data,deph//2,1,stride = 2, trainable=trainable) #用1*1的卷积代替下采样
    return data
...
def inference(input_tensor, regularizer = None, trainable=True):
    with slim.arg_scope([slim.conv2d,slim.max_pool2d],stride = 1,padding = 'SAME'):
        ...
        with tf.variable_scope("layer1-initconv"):
            ...
            data = slim.conv2d(input_tensor, CONV_DEEP , [7, 7], trainable=trainable)
